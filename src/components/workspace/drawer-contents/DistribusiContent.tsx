 ...
 */
export function DistribusiContent({
  token,
  onSuccess,
  onClose,
}: DistribusiContentProps) {
  ...
  const selectedInv = inventories.find(
    (inv) => inv.id.toString() === selectedInvId,
  );
  ...
  const handleFormSubmit: SubmitHandler<DistribusiFormInputs> = async (
    data,
  ) => {
    ...
    try {
      ...
      setApiError(
        err.message || "Terjadi kesalahan saat menyimpan distribusi.",
      );
    } finally {
  ...
  return (
    ...
    >
      <div className="flex-1 overflow-y-auto p-8 pt-6 flex flex-col gap-6">
        ...
        {/* Item Selection */}
        <div>
          <label className="block text-[11px] font-bold text-gray-800 tracking-wider uppercase mb-3">
            Pilih Barang
          </label>
          <select
          ...
          >
            <option value="">
              {isLoadingInv ? "Memuat..." : "Pilih Barang Inventaris..."}
            </option>
            {inventories.map((inv) => (
              <option key={inv.id} value={inv.id}>
                {inv.itemName} (Stok: {inv.stock} {inv.unit})
              </option>
            ))}
          </select>
          {errors.inventory_id && (
        ...
        {selectedInv && (
          <div className="bg-[#F6F9F9] rounded-2xl p-6 border border-[#E6F0F1]">
            <p className="text-[10px] font-bold text-[#0B648C] tracking-widest uppercase mb-4">
              Informasi Barang
            </p>
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-lg font-bold text-gray-900 mb-1">
                  {selectedInv.itemName}
                </h3>
              </div>
              <div className="text-right">
                ...
                <p className="text-base font-bold text-[#0B648C]">
                  {selectedInv.stock} {selectedInv.unit}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="relative">
            <input
              ...
              {...register("qty", {
                ...
                validate: (val) => {
                  if (selectedInv && val > selectedInv.stock)
                    return `Maksimal stok adalah ${selectedInv.stock}`;
                  return true;
                },
              })}
              placeholder="1"
        ...
        <div>
          ...
          />
          {errors.distributed_at && (
            <p className="text-red-500 text-[13px] font-medium mt-2">
              {errors.distributed_at.message}
            </p>
          )}
        </div>
        ...
        <div>
          <label className="block text-[11px] font-bold text-gray-800 tracking-wider uppercase mb-3">
            Penerima / Alokasi Target
          </label>
          <select
            {...register("target_recipient", {
              required: "Target penerima wajib diisi",
            })}
          ...
          >
            ...
            <option value="Unit Tujuuh">Unit Tujuh</option>
            <option value="Fasilitas Umum">Fasilitas Umum</option>
            <option value="Eksternal / Lainnya">Eksternal / Lainnya</option>
          </select>
          {errors.target_recipient && (
            <p className="text-red-500 text-[13px] font-medium mt-2">
              {errors.target_recipient.message}
            </p>
          )}
        </div>
        ...
        <div>
          <label className="block text-[11px] font-bold text-gray-800 tracking-wider uppercase mb-3">
            Keterangan Tambahan (Audit)
          </label>
          <textarea
            {...register("notes", {
              required: "Keterangan wajib diisi sebagai audit trail",
            })}
      ...
      <div className="p-8 pt-5 flex flex-col gap-3 bg-white mt-auto border-t border-gray-100">
        <button
          type="submit"
          disabled={isSubmitting || isLoadingInv}
          className="w-full py-4 bg-[#0B648C] text-white font-bold rounded-xl shadow-[0_4px_14px_rgba(11,100,140,0.3)] hover:shadow-[0_6px_24px_rgba(11,100,140,0.4)] hover:-translate-y-0.5 transition-all text-[15px] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            <>
        ...
        <button
          ...
          className="w-full py-4 text-gray-600 font-bold rounded-xl hover:bg-gray-50 transition-colors text-[15px] disabled:opacity-50"
        >
          Batal
        </button>
      </div>
