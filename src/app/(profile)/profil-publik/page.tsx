...
const MOCK_DONASI = [
  { id: 1, type: "Dana", nominal: "Rp 500.000", date: "24 Okt 2026", status: "Sukses" },
  { id: 2, type: "Barang", nominal: "Sepatu Sekolah (2 Pasang)", date: "10 Okt 2026", status: "Pending" },
  { id: 3, type: "Dana", nominal: "Rp 1.000.000", date: "01 Sep 2026", status: "Sukses" },
];

const MOCK_KUNJUNGAN = [
  { id: 1, purpose: "Kunjungan Personal", date: "15 Sep 2026", time: "10:00 WITA", status: "Sukses", pax: 2 },
  { id: 2, purpose: "Acara Syukuran", date: "05 Nov 2026", time: "14:00 WITA", status: "Menunggu Persetujuan", pax: 15 },
];
...
export default function ProfilPublikPage() {
  ...
  const renderStatusBadge = (status: string) => {
    switch(status) {
      case "Sukses":
        return <span className="px-3 py-1 bg-teal-50 text-teal-700 text-xs font-bold rounded-full flex items-center gap-1 w-fit"><MdCheckCircle /> {status}</span>;
      case "Pending":
      case "Menunggu Persetujuan":
        return <span className="px-3 py-1 bg-amber-50 text-amber-700 text-xs font-bold rounded-full flex items-center gap-1 w-fit"><MdPendingActions /> {status}</span>;
      default:
        return <span className="px-3 py-1 bg-gray-50 text-gray-700 text-xs font-bold rounded-full flex items-center gap-1 w-fit"><MdHistory /> {status}</span>;
    }
  };

  const HistoryCard = ({ title, subtitleTop, date, time, pax, status, icon: Icon, iconBg, iconColor }: any) => (
    <div className="bg-white/40 backdrop-blur-md p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-[0_4px_16px_-4px_rgba(0,0,0,0.05)] border-0 hover:bg-white/60 transition-colors">
      <div className="flex items-center gap-4">
        ...
        <div>
          {subtitleTop && <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">{subtitleTop}</p>}
          <h3 className="font-bold text-gray-900">{title}</h3>
          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500 mt-1">
            {date && <span className="flex items-center gap-1"><MdOutlineEventNote className="text-gray-400 text-sm" /> {date}</span>}
            {time && <span className="flex items-center gap-1"><MdOutlineAccessTime className="text-gray-400 text-sm" /> {time}</span>}
            {pax && <span className="flex items-center gap-1"><MdOutlinePerson className="text-gray-400 text-sm" /> {pax} Orang</span>}
          </div>
        </div>
      </div>
      <div>
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] pt-28 pb-20 px-6">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-white/80 backdrop-blur-xl shadow-sm rounded-3xl p-8 flex flex-col md:flex-row items-center md:items-start gap-8">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row items-center justify-center md:justify-start gap-3 mb-2">
              <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Ahmad Faisal</h1>
            ...
            <p className="text-gray-500 text-sm mb-6">Bergabung sejak Januari 2024</p>
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4">
              <div className="bg-teal-50/50 p-4 rounded-2xl flex items-center gap-4 flex-1 min-w-[150px]">
                ...
                <div>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Total Donasi</p>
              ...
              <div className="bg-blue-50/50 p-4 rounded-2xl flex items-center gap-4 flex-1 min-w-[150px]">
                ...
                <div>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Barang</p>
              ...
              <div className="bg-purple-50/50 p-4 rounded-2xl flex items-center gap-4 flex-1 min-w-[150px]">
                ...
                <div>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Kunjungan</p>
        ...
        <div className="pt-4">
          <h2 className="text-xl font-bold text-gray-900 mb-4 px-2">Aktivitas Saya</h2>
        ...
        <div className="bg-white/80 backdrop-blur-xl shadow-sm rounded-3xl p-8">
          ...
          {activeTab === "INFORMASI_UMUM" && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h2 className="text-xl font-bold text-gray-900 mb-6">Data Pribadi</h2>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                {/* Email - DISABLED */}
                <div>
                  <label className="block text-xs font-bold text-gray-500 tracking-wider uppercase mb-2">Alamat Email (Google Auth)</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  ...
                  <p className="text-[10px] text-gray-400 mt-1 italic">Email ini dikelola secara otomatis via login Google dan tidak dapat diubah.</p>
                </div>
                ...
                <div className="pt-4">
                  <button type="submit" className="px-8 py-3.5 bg-teal-700 text-white font-bold rounded-xl shadow-[0_4px_14px_rgba(15,118,110,0.3)] hover:-translate-y-0.5 transition-all">
                    Simpan Perubahan
                  </button>
                </div>
