...
import { FiSearch, FiCalendar, FiX, FiChevronRight, FiAlertTriangle, FiRefreshCw, FiBox } from "react-icons/fi";
...
function mapVisit(raw: any) {
  ...
  const isBEM = raw.user?.name?.toLowerCase().includes('bem') || raw.user?.name?.toLowerCase().includes('univ');
  ...
  const sessionStr = raw.capacity?.slot ? (slotMap[raw.capacity.slot] || raw.capacity.slot) : "Sesi Tidak Diketahui";
...
export default function ApprovalKunjunganPage() {
  ...
  const fetchVisits = useCallback(async () => {
    ...
    setFetchError(null);
    try {
      const res = await fetch(`${API_BASE}/kunjungan/pending`, {
    ...
    <div className="flex h-full w-full relative overflow-hidden">
      <div className={`flex-1 transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] p-8 overflow-y-auto ${isPanelOpen ? 'pr-[450px]' : ''}`}>
        <div className="max-w-5xl mx-auto w-full">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight mb-2">Validasi & Approval Kunjungan</h1>
            <p className="text-gray-500 text-lg">Tinjau pengajuan jadwal untuk mencegah konflik dan memastikan kenyamanan anak-anak.</p>
          </div>
          ...
          <div className="bg-teal-50/30 backdrop-blur-md rounded-2xl p-4 mb-8 shadow-sm">
            <div className="relative mb-4">
              ...
              <input type="text" placeholder="Cari pengajuan..." className="w-full pl-11 pr-4 py-3 bg-white rounded-xl text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500/20 shadow-[0_2px_10px_rgba(0,0,0,0.02)] transition-shadow border-none" />
            </div>
          ...
          {fetchError && (
            <div className="mb-6 flex items-center gap-3 px-5 py-4 bg-red-50 border border-red-200 rounded-2xl text-red-800 text-sm">
              ...
              <button onClick={fetchVisits} className="flex items-center gap-1.5 text-red-700 font-bold hover:text-red-900">
          ...
          <div className="flex flex-col gap-4">
            {isLoading ? (
              Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="flex items-center p-5 rounded-2xl bg-slate-50/70 animate-pulse gap-5">
              ...
              <div className="flex flex-col items-center justify-center py-20 text-gray-400 bg-slate-50 rounded-2xl border border-dashed border-gray-200">
                ...
                <p className="text-base font-medium text-gray-500">Tidak ada pengajuan kunjungan yang perlu disetujui saat ini.</p>
              </div>
            ) : (
              visits.map((item) => {
                ...
                return (
                  <div 
                    ...
                    className={`group flex items-center p-5 rounded-2xl cursor-pointer transition-all duration-300 ${
                      isSelected ? "bg-white ring-2 ring-teal-600 shadow-[0_8px_30px_rgb(0,0,0,0.06)]" : "bg-slate-50 hover:bg-white hover:shadow-[0_4px_20px_rgb(0,0,0,0.04)] hover:-translate-y-0.5"
                    }`}
                  >
                    <div className={`w-14 h-14 rounded-xl flex items-center justify-center mr-5 transition-colors ${isSelected ? 'bg-blue-100 text-blue-600' : item.iconBg}`}>
                      {item.icon}
                    </div>
                    ...
                    <div className="flex flex-col items-end justify-center gap-2">
                      ...
                      <FiChevronRight className={`text-xl transition-transform duration-300 ${isSelected ? "text-teal-600 translate-x-1" : "text-gray-300 group-hover:text-teal-400 group-hover:translate-x-1"}`} />
                    </div>
                  </div>
                );
              })
